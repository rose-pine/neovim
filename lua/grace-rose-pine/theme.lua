...
function M._load(options)
	local h = require('grace-rose-pine.util').highlight
	...
	h('StorageClass', { fg = p.pine })
	h('String', { fg = p.gold })
	h('Structure', { fg = p.moss })
	h('Tag', { fg = p.rose })
	h('Todo', { fg = p.iris })
	h('Type', { fg = p.foam })
	h('Typedef', { link = 'Type' })
	...
	h('@interface', { fg = p.moss, italic = true })
	...
	h('@storageclass.lifetime', { fg = p.pine, italic = true })
